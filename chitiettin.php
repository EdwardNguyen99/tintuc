...
<div class="row">
    <div class="post single">
        ...
        <ul class="post_details clearfix">
            <li class="detail category"> <a href="<?=BASE_DIR?>bv/<?=$row['TieuDe_KhongDau'];?>.html" title="<?=$row['Ten']?>"><?=$row['Ten']?></a></li>
...
<?php $tag_arr = explode(',',trim($row['tags']));?>
<?php if(trim($row['tags'])!=""){?>
    <div class="row page_margin_top">
        <ul class="taxonomies tags left clearfix">
            <?php for($i=0; $i<count($tag_arr);$i++){?>
                <li>
                    <a href="#" title="<?=$tag_arr[$i]?>"><?=$tag_arr[$i]?></a>
                </li>
            <?php }?>
        </ul>
    </div>
        ...
        <ul class="blog horizontal_carousel autoplay-1 scroll-1 navigation-1 easing-easeInOutQuint duration-750">
            ...
            <?php while ($row = $kq->fetch_assoc()) {?>
                <li class="post tintieptheo">
                    ...
                    <h5><a href="<?=BASE_DIR?>bv/<?=$row['TieuDe_KhongDau'];?>.html" title="<?=$row['TieuDe']?> "><?=$row['TieuDe']?> </a></h5>
