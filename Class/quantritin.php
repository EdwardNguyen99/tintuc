...
class quantritin extends goc  {
    ...
    function TheLoai_ChiTiet($idTL){
        ...
        $kq = $this->db->query($sql) ;
        if(!$kq) die( "Lỗi trong hàm: ".__FUNCTION__." SQL=".$sql." "."\n".$this-> db->error);
        return $kq; 
     }
    ...
    //-	Viết 1 hàm trả về số tin trong thể loại.  
    //-	Trước khi gọi hàm TheLoai_Xoa, hãy gọi hàm đếm tin, nếu >0 thì: báo lỗi 
    function DemTinTrongTheLoai($idTL){
        $sql="SELECT count(*) as soTin FROM tin where idTL=$idTL";
        $kq = $this->db->query($sql) ;
        $row = $kq->fetch_assoc();
        return $row['soTin']; 
     }
     ...
     function LoaiTinTrongTheLoai($idTL){
        $sql="SELECT idLT,Ten 
        FROM loaiTin 
        WHERE idTL=$idTL or $idTL=-1 ORDER BY ThuTu";
        $kq = $this->db->query($sql) ;
        if(!$kq) die( "Lỗi trong hàm: ".__FUNCTION__." SQL=".$sql." "."\n".$this-> db->error);
        return $kq; 
     }

     function LoaiTinTrongTheLoaiA($idTL){
      $sql="SELECT idLT,Ten, loaitin.ThuTu,loaitin.AnHien, 
      Ten_KhongDau, loaitin.lang, TenTL 
      FROM loaitin, theloai 
    ...
    function DemTinTrongLoaiTin($idLT){
        $sql="SELECT count(*) as soTin FROM tin where idLT=$idLT";
        $kq = $this->db->query($sql) ;
        $row = $kq->fetch_assoc();
        return $row['soTin']; 
     }
   ...
   function ListTin( $idTL, $idLT,$lang ){
   ...
   function YKienTrongTin($idTin){
      $sql="SELECT count(*) as soYKien FROM ykien where idTin=$idTin";
      $kq = $this->db->query($sql) ;
      $row = $kq->fetch_assoc();
      return $row['soYKien']; 
   }
