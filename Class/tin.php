...
class tin extends goc{
    function TinNoiBat($from, $sotin, $lang){
        ...
        from tin, loaitin
        where tin.idLT = loaitin.idLT and tin.AnHien=1 and TinNoiBat =1 and
        tin.lang='$lang'
    ...
    function ChiTietTin($idTin)  {
        ...
        tin.idLT, Ten, tin.idTL, TenTL, tags
    ...
    function LuuYKien($idTin, $hoten,$email, $noidung, $loi){
        ...
        if ($hoten=="") 
            $loi.="Ban chưa nhập họ tên<br>";
        if ($email=="") 
            $loi.="Ban chưa nhập email<br>";
        if ($noidung=="") 
            $loi.="Ban chưa nhập nội dung ý kiến<br>";
        if (strlen($noidung)<10) 
            $loi.="Nội dung ý kiến quá ngắn<br>";
        if ($loi!="") 
            return FALSE;
    ...
    */
    ...
    function TinTrongLoai($idLT , $pageNum, $pageSize, &$totalRows ){//& de lấy giá trị gán ra ngoai
    ...
    function TimKiem($tukhoa, &$totalRows, $pageNum=1, $pageSize=5){
     ...
     function getTitle($p=''){
        ...
        elseif ($p=="cat"){
            ...
            $kq = $this->db->query("select Ten from loaitin where Ten_KhongDau= '$Ten_KhongDau'"); //idLT=$id");
            if(!$kq) die( $this-> db->error);
            if ($kq->num_rows<=0) 
               return "Tin tức tổng hợp";
            $row_kq = $kq->fetch_row();
    ...
    function LayidLT($Ten_KhongDau){
        $Ten_KhongDau = trim(strip_tags($Ten_KhongDau));
